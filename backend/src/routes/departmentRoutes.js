...
const { createDepartment, updateDepartment } = require("../validations/departmentValidation");

const router = express.Router();

router.use(auth);
router.get("/", authorize(ROLES.ADMIN, ROLES.HR), paginationQuery, validate, controller.list);
router.post("/", authorize(ROLES.ADMIN), createDepartment, validate, controller.create);
router.get("/:id", authorize(ROLES.ADMIN, ROLES.HR), idParam(), validate, controller.getById);
router.put("/:id", authorize(ROLES.ADMIN), idParam(), updateDepartment, validate, controller.update);
